...
class BCASL:
    ...
    IMPORTANT: N'utilise PAS de multiprocessing pour éviter les segfaults avec Qt.
    Utilise du threading simple pour l'exécution non-bloquante.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # Sandbox settings (BCASL n'utilise pas de multiprocessing)
    ...
    def run_pre_compile(
        ...
        Utilise du threading simple (pas de multiprocessing) pour éviter les segfaults.
        """
        if ctx is None:
        ...
        # Exécution séquentielle avec threading (non-bloquant)
        def _run_plugins():
            for pid in order:
                rec = active_items[pid]
                plg = rec.plugin
                ...
                try:
                    # Appeler on_pre_compile avec timeout optionnel
                    ...
                    if timeout and timeout > 0:
                        ...
                        th = threading.Thread(
                            target=_call, name=f"BCASL-{pid}", daemon=True
                        )
                        th.start()
                        th.join(timeout)
                        if th.is_alive():
        ...
        worker_thread = threading.Thread(
            target=_run_plugins, name="BCASL-Worker", daemon=True
        )
        worker_thread.start()
